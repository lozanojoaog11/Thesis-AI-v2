...
export const Phase5_SelectionStep: React.FC<SelectionProps> = ({ thesisData, updateThesisData, onNext, onBack }) => {
  ...
  return (
    <div className="animate-fadeIn w-full max-w-4xl space-y-12">
      <div>
        <h2 className="text-3xl font-sans text-brand-light font-bold">{t('Phase 5: Final Selection')}</h2>
        <p className="text-gray-400 font-sans mt-2 mb-8">{t('The time for divergence is over. Converge on the single strongest business model and justify your choice.')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {thesisData.businessModels.map((model, index) => {
          ...
          return (
            ...
            >
              <h3 className="font-bold text-lg text-brand-light mb-2">{t('Hypothesis #{{index}}', { index: index + 1 })}</h3>
              <p className="text-sm italic text-gray-300 mb-4">"{hypothesis.idea}"</p>
              <h4 className="font-mono text-sm text-gray-400">{t('UVP:')}</h4>
              <p className="text-sm mb-2">{model.leanCanvas.uniqueValueProposition}</p>
              <h4 className="font-mono text-sm text-gray-400">{t('Unfair Advantage:')}</h4>
      ...
      {selectedModelIndex !== -1 && (
        <div className="animate-fadeIn space-y-4">
          <label className="text-xl font-sans text-brand-light block">{t('Justify your selection:')}</label>
          <TextArea 
            placeholder={t('Synthesize your reasoning based on all previous phases. Why is this the winning model?')}
            value={justification}
